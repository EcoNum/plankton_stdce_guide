...
Les tuniciers constituent un sous-phylum des Chordés. Leur nom provient de la tunique produite par l'épiderme, les recouvrant. Le stade larvaire de ces organismes possède une notochorde (dépourvue au stade adulte) [@ruppert2004].
...
Ces organismes sont transparents ce qui se marque par le jaune très claire sur les vignette. Le cerclage de bande musculaire présent chez les Doliolida n'est pas présent. Il y a cependant bien la présence de muscle. On ne retrouve pas la forme de tonneaux des Doliolida ( @sec-doliolida ) .

![Vignette (A) et Image (B) de Salpida [@caicci2013].](images/03plankton_gelatinous/salpida.png){width="250"}

La @fig-vig-salpidae présente plusieurs vignettes de Salpidae.

```{r}
#| label: "fig-vig-salpidae"
#| fig-cap: "Vignettes de Salpidae"
plot_vignettes(vigns, group = "Salpidae")
```

Ce groupe est similaire au groupe `salpida` dans le ZooScanNet (@fig-zsn-salpida) .
...
Images de Salpida du ZooScanNet [@elineau2018].
:::
...
Les pyrosomes peuvent former de grandes colonies tubulaires de plusieurs mètres. Ils sont constitués de milliers de blastozoïdes dont la tunique est fusionnée et le siphon buccal s'ouvre au centre du tube [@caicci2013].

![Image de pyrosomatida [@caicci2013].](images/03plankton_gelatinous/pyrosomatida.png){#fig-pyrosomatida}

Dans le ZooScanNet, on les retrouve très souvent sous forme de colonies (@fig-zsn-pyrosomatida). Le pharynx est discernable sur chaque image.

::: {#fig-zsn-pyrosomatida}
...
Images de Pyrosomatida du ZooScanNet [@elineau2018].
:::

#### Doliolida {#sec-doliolida}

Ces organismes ont l'aspect d'un tonneau symétrique doté d'un cerclage de plusieurs cercles (@fig-doliolida). Les cerclages sont des muscles entourant le corps de l'organisme.

![Vignette (A) et image (B) de Doliolida [@caicci2013].](images/03plankton_gelatinous/doliolida.png){#fig-doliolida width="250"}

La @fig-vig-doliolida présente plusieurs vignettes de Sagittoidea. Cet organisme est transparent comme l'indique les parties jaunes claires sur les vignettes. Les bandes musculaires qui entoure l'organisme apparaissent plus foncée. La cavité plus dense est l'estomac.

```{r}
#| label: "fig-vig-doliolida"
#| fig-cap: "Vignettes de Doliolida"
plot_vignettes(vigns, group = "Doliolida", nx = 3,ny = 2)
```

Ce groupe est similaire au groupe `doliolida` dans le ZooScanNet ( @fig-zsn-doliolida ).
...
Images de Doliolida du ZooScanNet [@elineau2018]
:::

### Appendicularia

Les Appendicularia conserve leur queue à l'état adulte. Ce caractère néoténique les rend facilement discernable. Ces organismes sont divisée en deux parties : la tête et le tronc ( @fig-appendicularia ).

![Vignette d'Appendicularia: A tête, B tronc [@quivythomas2018].](images/03plankton_gelatinous/appendicularia_comp.png){#fig-appendicularia}

#### Copelata

##### Oikopleuridae

Au niveau de la tête les glandes, les organes (intestin, oeisophage, glandes,...)sont de grandes tailles et donne une coloration très foncée à la tête de l'organisme. Les cellules subchrodal qui entoure la notochorde donne une impression plus dense de la queue. (@fig-oikopleuridae)

![Vignette (A) et image (B) d'Oikopleuridae [@hecq2014]](images/03plankton_gelatinous/oikopleuridae.png){#fig-oikopleuridae width="250"}

La @fig-vig-oikopleuridae présente plusieurs vignettes d'Oikopleuridae.
...
La tête de ces organismes est moins foncée sur les vignettes (@fig-fritillariidae). On retrouve un forme ressemblant à un marteau ou un pioche. La queue semble moins dense mais assez large.

![Vignette (A) et image de Fritillariidae [@hecq2014]](images/03plankton_gelatinous/frittillariidae.png){#fig-fritillariidae}

La @fig-vig-fritillariidae présente plusieurs vignettes de Fritillariidae.

```{r}
#| label: "fig-vig-fritillariidae"
#| fig-cap: "Vignettes de Fritillariidae"
plot_vignettes(vigns, group = "Fritillariidae")
```

#### Appendicularia_tail

Lors des traits de filets, l'organisme est souvent endommagé. On retrouve de ce fait dans les échantillons des queues d'appendiculaire (@fig-appendicularia-tail). Les bandes foncées entourant la notochorde est généralement visible [@gorsky2017].

![Vignette d'une queue d'appendicularia : A masse musculaire, B notochorde [@quivythomas2018].](images/03plankton_gelatinous/appendicularia_tails.png){#fig-appendicularia-tail width="250"}

La @fig-vig-appendicularia-tail présente plusieurs vignettes de queue d'Appendiculaira.

```{r}
#| label: "fig-vig-appendicularia-tail"
#| fig-cap: "Vignettes de queues d'Appendicularia."
plot_vignettes(vigns, group = "Appendicularia_tail")
```

Ce groupe est similaire au groupe `tail__appendicularia` dans le ZooScanNet ( @fig-zsn-appendicularia-tail ).
...
Images de queues d'Appendicularia du ZooScanNet [@elineau2018].
:::
